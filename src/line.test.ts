...
describe("lerpLine", () => {
  ...
  describe("edge cases", () => {
    ...
    test("negative coordinates - horizontal line", () => {
      ...
      expect(result.length).toBe(4);
      expect(vectorsApproxEqual(result[0], start)).toBe(true);
      expect(vectorsApproxEqual(result[1], createVector(-66.6667, -50))).toBe(
        true
      );
      expect(vectorsApproxEqual(result[2], createVector(-33.3333, -50))).toBe(
        true
      );
  ...
  describe("mathematical correctness", () => {
    ...
    test("verify 3D interpolation", () => {
      ...
      expect(result.length).toBe(3);
      expect(vectorsApproxEqual(result[0], start)).toBe(true);
      expect(vectorsApproxEqual(result[1], createVector(50, 25, 12.5))).toBe(
        true
      );
      expect(vectorsApproxEqual(result[2], end)).toBe(true);
    });
